...
class Poirot:
    ...
    def _detect_system_info(self) -> None:
        ...
        self._system_info.co2_info.date = co2_info.date
    ...
    def stop_profiling(self) -> None:
        ...
        if self._verbose:
            print(
                ...
                f"CPU Energy: {call.data.cpu_energy_uj:.4f}uJ | "
                f"GPU Energy: {call.data.gpu_energy_uj:.4f}uJ | "
                f"Total Energy: {call.data.total_energy_uj:.4f}uJ | "
                f"CO2: {call.data.co2_ug:.4f}ug",
                file=sys.stderr,
            )
    ...
    def print_system_info(cls) -> None:
        ...
        print(f"CO2: {info.co2_info.co2_factor_kg_per_kwh:.4f} kg/kWh", file=sys.stderr)
        print("=" * 64, file=sys.stderr)
