...
class PowerEstimator:
    ...
    def rapl_available(self) -> bool:
        ...
        """
        return os.path.exists(self.INTEL_RAPL_PATH) or os.path.isdir(self.AMD_RAPL_PATH)
    ...
    def read_intel_rapl_power_limit_w(self) -> float:
        ...
        Returns:
            Power limit in watts or 0.0 if not available.
        """
        # Try both constraints like C++ does
        intel_rapl_paths = [
    ...
    def read_watts_per_ghz(self) -> float:
        ...
        """
        # Try to get actual current power from hwmon (not power limit)
    ...
    def read_hwmon_tdp_watts(self) -> float:
        ...
        def find_power_cap(base_path: str, name: str) -> bool:
            nonlocal result
            if name.startswith("rapl"):
                power_cap_path = os.path.join(base_path, "power1_cap")
                power_uw = SysfsReader.read_long(power_cap_path)
                if power_uw > 0:
                    result = power_uw / 1_000_000.0
                    return True
            return False

        self._hwmon_scanner.iterate_devices(find_power_cap)
        return result
    ...
    def read_thermal_tdp_watts(self) -> float:
        ...
        Returns:
            TDP in watts or 0.0 if not available.
        """
        # Try /sys/devices/system/cpu/cpu0/thermal_throttle
        path = "/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_max_freq"
        max_freq_khz = SysfsReader.read_long(path)
        if max_freq_khz > 0:
            # Estimate TDP from max frequency and cores
            max_freq_ghz = max_freq_khz / 1_000_000.0
            return self._cpu_cores * max_freq_ghz * FALLBACK_WATTS_PER_GHZ
